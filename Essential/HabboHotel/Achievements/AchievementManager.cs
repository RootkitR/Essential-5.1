...
namespace Essential.HabboHotel.Achievements
{
	internal class AchievementManager
	{
        ...
        public Dictionary<uint, Achievement> GetAchievements()
        {
            return dictionary_0;
        }
		...
		public uint GetBadgeId(string string_0)
		{
		...
		public bool HasAchievement(GameClient Session, uint Id, int Level)
		{
			return Session.GetHabbo().dictionary_0.ContainsKey(Id) && Session.GetHabbo().dictionary_0[Id] >= Level;
		}
		public static ServerMessage SerializeAchievements(GameClient Session)
		{
            ...
            ServerMessage Message = new ServerMessage(Outgoing.AchievementList); // Updated
			...
			foreach (KeyValuePair<uint, Achievement> current in AchievementManager.dictionary_0)
			{
				if (!(current.Value.Type == "hidden"))
				{
                    ...
                    int Pixels = AchievementManager.CalculatePixels(current.Value.Dynamic_badgelevel, current.Value.PixelMultiplier, num3);
					Message.AppendUInt(current.Value.Id);
					Message.AppendInt32(num3);
					Message.AppendStringWithBreak(AchievementManager.GetBadgeName(current.Value.BadgeCode, num3, current.Value.DynamicBadgeLevel));
        ...
        public void addAchievement(GameClient Session, uint uint_0, int int_0)
        {
            ...
            {
                Achievement @class = AchievementManager.dictionary_0[uint_0];
                if (@class != null && !this.HasAchievement(Session, @class.Id, int_0) && int_0 >= 1 && int_0 <= @class.Levels)
                {
                    int num = AchievementManager.CalculatePixels(@class.Dynamic_badgelevel, @class.PixelMultiplier, int_0);
                    int num2 = AchievementManager.CalculatePixels(@class.ScoreBase, @class.PixelMultiplier, int_0);
                    ...
                    Session.GetHabbo().GetBadgeComponent().SendBadge(Session, AchievementManager.GetBadgeName(@class.BadgeCode, int_0, @class.DynamicBadgeLevel), true);
                ...
                IL_346:
                    ...
                    int Need = GetNeedd(@class.Id, int_0, Session.GetHabbo());
                    int Have = GetHavee(@class.Id, Session.GetHabbo());

                    /*ServerMessage Message2 = new ServerMessage(Outgoing.AchievementProgress);
                    Message2.AppendUInt(@class.Id);
                    Message2.AppendInt32(int_0);
                    Message2.AppendStringWithBreak(AchievementManager.GetBadgeName(@class.BadgeCode, int_0, @class.DynamicBadgeLevel));
                    Message2.AppendInt32(Need);
                    Message2.AppendInt32(num2);
                    Message2.AppendInt32(num);
                    Message2.AppendInt32(0); // ?
                    Message2.AppendInt32(Have);
                    Message2.AppendBoolean(true);
                    if (int_0 > 1)
                    {
                        Message2.AppendStringWithBreak(AchievementManager.GetBadgeName(@class.BadgeCode, int_0 - 1, @class.DynamicBadgeLevel));
                    }
                    else
                    {
                        Message2.AppendStringWithBreak("");
                    }
                    Message2.AppendStringWithBreak(@class.Type);
                    Message2.AppendInt32(@class.Levels);
                    Message2.AppendInt32(0);
                    Session.SendMessage(Message2);*/
                    
                    AchievementManager.SerializeAchievements(Session);
                    ServerMessage Message = new ServerMessage(Outgoing.UnlockAchievement); // Updated
                    Message.AppendUInt(@class.Id);
                    Message.AppendInt32(int_0);
                    Message.AppendInt32(144);
                    Message.AppendStringWithBreak(AchievementManager.GetBadgeName(@class.BadgeCode, int_0, @class.DynamicBadgeLevel));
                    ...
                    if (int_0 > 1)
                    {
                        Message.AppendStringWithBreak(AchievementManager.GetBadgeName(@class.BadgeCode, int_0 - 1, @class.DynamicBadgeLevel));
                    }
                    else
                    {
		...
		public static int CalculatePixels(int int_0, double double_0, int int_1)
		{
			return (int)((double)int_0 * ((double)int_1 * double_0));
		}
		public static string GetBadgeName(string string_0, int int_0, bool bool_0)
		{
			if (!bool_0)
			{
				return string_0;
        ...
        public static int GetNeed(uint AchievementId, int AchLevel, Users.Habbo Habbo)
        {
            if (AchievementId == 4)
            {
                switch (AchLevel)
        ...
        internal static int GetHave(uint AchievementId, Users.Habbo Habbo)
        {
            switch (AchievementId)
            {
                ...
                case 24:
                    return Habbo.GetMessenger().hashtable_0.Count;
            }
            return 0;
        }
        internal int GetLevel(uint AchievementId,GameClient Session)
        {
            if(Session.GetHabbo().dictionary_0.ContainsKey(AchievementId))
                return Session.GetHabbo().dictionary_0[AchievementId];
            return 0;
        }
        internal int GetLevels(uint AchievementId)
        {
            if (dictionary_0.ContainsKey(AchievementId))
                return dictionary_0[AchievementId].Levels;
            return 0;
        }
        internal bool HasAchievements(Dictionary<uint, int> dict, GameClient Session)
        {
            foreach(KeyValuePair<uint,int> kvp in dict)
            {
                if (!HasAchievement(Session, kvp.Key, kvp.Value))
                    return false;
            }
            return true;
        }
